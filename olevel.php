...
<body>
                                                      ...
                                                      if(result=="ok"){
                                                          ...
                                                          function(){
 		                                                                 ...
 		                                                                 $.ajax({
 		 	                                                                      type :'POST',
                                                                                    url: "process.php",
                                                                                   data: {loadlevel:optionValue,userpin:pin,userserial:serial},
                                                                                success: function(result) { 
                                                                                	                        if (result=='No') {
                                                                                	                        	                $("#errors1").html(""); 
                                                                                	                        	                $("#errors2").html(""); 
 ...
 $(document).on("click", ".Olevel", function () {
      ...
      var tos = document.getElementById('grade').value;
      var institution = document.getElementById('examnumber').value;
      var pin = document.getElementById('examdate').value;
     var serial = document.getElementById('subjects').value;

	//  console.log({serial, tos});
   

			$.ajax ({
            type : 'POST',
               url: "process.php",
             data: {examtype:name,sitting:froms,grade:tos,exam:institution,examdate:pin,subjects:serial},              
          success: function(result) {
                         
          	                         $("#errors1").html(result);
          	                         $("#errors2").html("<a  class='btn btn-default' href='employment.php'><span class='glyphicon glyphicon-log-in'></span> &nbsp;Submit Courses</a>");
                                     swal({title: "Successful!", text: "Olevel Results Saved!!.", type: "success"});
...
<!-- gallery -->
	<div class="agileits-services text-center py-5">
		<div class="container py-md-4 mt-md-3">
			<h3 class="heading-agileinfo">Stage 6 (O'Level Results)<span>Click on Browse button below to select your picture and should not be more than 20MB</span></h3>
...
<select style='height:37px;width:100%;border:1px solid black;' name="examtype"  id='examtype' > 
  ...
  <div class="input-group" style="margin-bottom:10px">
    <span class="input-group-addon">Center & Exam Number</span>
...
<select style='height:37px;width:100%;border:1px solid black;' name="sitting"  id='sitting' > 
  ...
  <div class="input-group" style="margin-bottom:10px;">
  	   <input type="checkbox" class="form-control" name="1st">&nbsp;

    <span class="input-group-addon" style='height:34px;'>Subject 1st Sitting</span>
  <select style='height:34px;width:100%;border:1px solid black;' name="subjects"  id='subjects' > 
  ...
  <select style='height:37px;width:100%;border:1px solid black;' name="grade"  id='grade' >          
...
<div class="input-group" style="margin-bottom:10px;">
    ...
    <span class="input-group-addon">Subject 1st Sitting</span>
  <select style='height:37px;width:100%;border:1px solid black;' name="subjects"  id='subjects' > 
