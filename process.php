...
try {
    ...
    if (isset($_FILES['file2']['name']) && $_POST['document']) {
        ...
        if (isset($_POST['ids'])) {
            ...
            echo "updated"; // Echo upload status
            exit;
        }
    ...
    if (isset($_POST['dstatus'])) {
        ...
        $checks = mysqli_num_rows($resul);
        if ($checks == 0) {
            $getMaxMatricQuery = "SELECT COALESCE(MAX(CAST(SUBSTRING(Applno, 8) AS UNSIGNED)), 0) AS maxMatric
            FROM declared
            WHERE SUBSTRING(Applno, 1, 7) = 'NRS/24/'";
